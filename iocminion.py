...
class iocMinion():
  ...
  def print_iocs(self,ioc_data,toFile,outFile):
    fileOut = []
...
def main():
  ...
  group.add_argument('--pdf',help='process pdf on an url or a pdf on the filesystem. make sure filename doesnt contain spaces',nargs=1,required=False)
  group.add_argument('--email',help='looks for iocs on gmail inbox. username and password are required (username pass)',nargs=2)
  group.add_argument('--write',help='write Results to a file',nargs=1,required=False )
  ...
  if args.write:
    iocObj.print_iocs(ioc_data,True,args.write[0])
  else:
    iocObj.print_iocs(ioc_data,False,None)
